...
const Slider_3_Img2 =  ({ settings , data }) => {
          ...
          <Slider {...settings} className='container' >
            {Array(len).fill(1).map((e,index)=>(
                  <div className="box" key={index} > 
                    <div className="cover" data-aos="fade-up" data-aso-delay={`${index}00`}> <img src={data[index]}  /> </div>
                    <div className="cover" data-aos="fade-up" data-aso-delay={`${index}00`}> {data[index + len] && <img src={data[index + len]} />} </div>
              </div>
              ))}
